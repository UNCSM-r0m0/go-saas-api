...
func TestGenerateAndValidateToken(t *testing.T) {
	...
	token, err := mgr.GenerateToken(userID, tenantID, "member", time.Hour)
...
func TestValidateToken_WrongSecret(t *testing.T) {
	...
	token, err := mgr1.GenerateToken("user-1", "tenant-1", "member", time.Hour)
	require.NoError(t, err)
...
func TestValidateToken_Expired(t *testing.T) {
	...
	token, err := mgr.GenerateToken("user-1", "tenant-1", "member", -time.Hour)
	require.NoError(t, err)
...
func TestParseUserID(t *testing.T) {
	...
	token, err := mgr.GenerateToken(userID, uuid.NewString(), "admin", time.Hour)
	require.NoError(t, err)
