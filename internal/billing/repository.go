...
type UsageRepository interface {
	...
	IncrementDailyUsage(ctx context.Context, userID uuid.UUID, date time.Time, tokensIn, tokensOut int, costUSD float64) error
	GetUsageStats(ctx context.Context, userID uuid.UUID, from, to time.Time) (*UsageStats, error)
}
