...
import { Plus, Sparkles, Square, PanelLeft, Sun, Moon, Terminal, Bot, PanelRightClose, X } from 'lucide-react';
...
import { ApiIntegrationFlowPanel } from '../chat/ApiIntegrationFlowPanel';
...
export const MainLayout: React.FC<MainLayoutProps> = ({
  ...
  const isApiFlow = currentChat?.metadata?.flow === 'api_integration';
  ...
  return (
    <div className="h-[100dvh] w-full overflow-hidden bg-[var(--bg-primary)] text-[var(--text-primary)]">
      <div className="h-full flex min-h-0">
        ...
        <main className="flex-1 min-w-0 flex flex-col relative bg-[var(--bg-primary)]">
          ...
          <header className="flex-shrink-0 flex items-center justify-between px-4 py-2">
            ...
            <div className="flex items-center gap-1">
              ...
              {/* Flow Panel Toggle - Mobile only */}
              {isApiFlow && (
                <button
                  onClick={() => setIsMobileFlowOpen(true)}
                  className="md:hidden p-2 rounded-lg text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/[0.04] transition-colors"
        ...
        {/* API Integration Flow Panel - Desktop sidebar */}
        {isApiFlow && (
          <aside className="hidden md:flex w-80 flex-shrink-0 flex-col border-l border-[var(--border-subtle)] bg-[var(--bg-secondary)] h-full">
            <ApiIntegrationFlowPanel
              metadata={currentChat?.metadata}
              onSendMessage={handlePromptClick}
              isStreaming={isStreaming}
            />
          </aside>
        )}
      ...
      {/* API Integration Flow Panel - Mobile drawer */}
      <AnimatePresence>
        {isApiFlow && isMobileFlowOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            <motion.div
              ...
              exit={{ x: '100%' }}
              transition={{ type: 'spring', damping: 25, stiffness: 180, mass: 0.8 }}
              className="absolute right-0 top-0 bottom-0 w-80 bg-[var(--bg-secondary)] border-l border-[var(--border-subtle)] flex flex-col"
            >
              <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border-subtle)]">
                <span className="text-sm font-semibold text-[var(--text-primary)]">Flow</span>
                <button
                  onClick={() => setIsMobileFlowOpen(false)}
                  className="p-1.5 rounded-lg text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/[0.04] transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
